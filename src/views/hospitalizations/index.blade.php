    ...
    <div class="container mx-auto px-4">
        <div class="bg-white shadow-md rounded my-6">
            ...
            <div class="px-12 pb-10">
                <div class="text-center text-5xl mb-8 font-bold">Hospitalizations</div>

                <form action="/controllers/hospitalizations/index.php" method="get" class="mb-4">
                    <div class="space-y-2">
                        Filter by
                        <input type="text" name="patient" placeholder="Patient" class="border rounded px-3 py-2 mr-2">
                        <input type="text" name="room" placeholder="Room" class="border rounded px-3 py-2 mr-2">
                ...
                <!-- Create form -->
                <form action="/controllers/hospitalizations/store.php" method="post" class="mb-4">
                    <div class="space-y-2">
                        <input type="text" name="patient" placeholder="Patient" class="border rounded px-3 py-2 mr-2">
                        <input type="text" name="room" placeholder="Room" class="border rounded px-3 py-2 mr-2">
                        <input type="datetime-local" name="dt_time_start" placeholder="Appointment Start Time" class="border rounded px-3 py-2 mr-2">
                ...
                <div class="overflow-x-auto w-full">
                    <table class="w-full text-md bg-white shadow-md rounded mb-4">
                        <thead>
                            <tr class="border-b">
                                @if (isset($hospitalizations[0]))
                                    @foreach ($hospitalizations[0] as $key => $value)
                        ...
                        <tbody>
                            @foreach ($hospitalizations as $hospitalization)
                                <tr class="border-b hover:bg-orange-100">
                                    @foreach ($hospitalization as $value)
                                    ...
                                    <td class="text-red-500 p-3 px-5">
                                        <form id="delete-form" action="/controllers/hospitalizations/destroy.php" method="post" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                            <input type="hidden" name="hospitalizationid" value="{{ $hospitalization['hospitalizationid'] }}">
                                ...
                                <tr class="border-b hidden">
                                    <form id="edit-form" action="/controllers/hospitalizations/update.php" method="post">
                                        @foreach ($hospitalization as $key => $value)
                                            @if (strpos($key, 'link-') === 0)
                                                <td class="p-3 px-5"></td>
                                            @elseif ($key != 'hospitalizationid')
                                                <td class="p-3 px-5">
