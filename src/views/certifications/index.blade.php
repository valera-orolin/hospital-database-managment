    ...
    <div class="container mx-auto px-4">
        <div class="bg-white shadow-md rounded my-6">
            ...
            <div class="px-12 pb-10">
                <div class="text-center text-5xl mb-8 font-bold">Certifications</div>

                <form action="/controllers/certifications/index.php" method="get" class="mb-4">
                    <div class="space-y-2">
                        Filter by
                        ...
                        <input type="date" name="date_certificationstart" placeholder="Certification Start Date" class="border rounded px-3 py-2 mr-2">
                        <input type="date" name="date_certificationend" placeholder="Certification End Date" class="border rounded px-3 py-2 mr-2">
                    </div>
                ...
                <!-- Create form -->
                <form action="/controllers/certifications/store.php" method="post" class="mb-4">
                    <div class="space-y-2">
                        <input type="text" name="doctor" placeholder="Doctor" class="border rounded px-3 py-2 mr-2">
                        <input type="text" name="procedure" placeholder="Procedure" class="border rounded px-3 py-2 mr-2">
                        <input type="date" name="date_certificationstart" placeholder="Certification Start Date" class="border rounded px-3 py-2 mr-2">
                ...
                <div class="overflow-x-auto w-full">
                    <table class="w-full text-md bg-white shadow-md rounded mb-4">
                        <thead>
                            <tr class="border-b">
                                @if (isset($certifications[0]))
                                    @foreach ($certifications[0] as $key => $value)
                            ...
                            @foreach ($certifications as $certification)
                                <tr class="border-b hover:bg-orange-100">
                                    ...
                                    <td class="text-red-500 p-3 px-5">
                                        <form id="delete-form" action="/controllers/certifications/destroy.php" method="post" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                            <input type="hidden" name="doctor" value="{{ $certification['doctor'] }}">
                                            <input type="hidden" name="procedure" value="{{ $certification['procedure'] }}">
                                ...
                                <tr class="border-b hidden">
                                    <form id="edit-form" action="/controllers/certifications/update.php" method="post">
                                        @foreach ($certification as $key => $value)
                                            @if (strpos($key, 'link-') === 0)
                                                <td class="p-3 px-5"></td>
                                            @elseif ($key != 'doctor' && $key != 'procedure')
                                                <td class="p-3 px-5">
