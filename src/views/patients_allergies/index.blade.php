    ...
    <div class="container mx-auto px-4">
        <div class="bg-white shadow-md rounded my-6">
            ...
            <div class="px-12 pb-10">
                <div class="text-center text-5xl mb-8 font-bold">Patients - Allergies</div>

                <!-- Filter and sort form -->
                <form action="/controllers/patients_allergies/index.php" method="get" class="mb-4">
                    <div class="space-y-2">
                        Filter by
                        <input type="text" name="patient" placeholder="Patient" class="border rounded px-3 py-2 mr-2">
                        <input type="text" name="allergy" placeholder="Allergy" class="border rounded px-3 py-2 mr-2">
                ...
                <!-- Create form -->
                <form action="/controllers/patients_allergies/store.php" method="post" class="mb-4">
                    <div class="space-y-2">
                        <input type="text" name="patient" placeholder="Patient" class="border rounded px-3 py-2 mr-2">
                        <input type="text" name="allergy" placeholder="Allergy" class="border rounded px-3 py-2 mr-2">
                        <input type="date" name="date_diagnosed" placeholder="Date Diagnosed" class="border rounded px-3 py-2 mr-2">
                ...
                <div class="overflow-x-auto w-full">
                    <table class="w-full text-md bg-white shadow-md rounded mb-4">
                        <thead>
                            <tr class="border-b">
                                @if (isset($patients_allergies[0]))
                                    @foreach ($patients_allergies[0] as $key => $value)
                        ...
                        <tbody>
                            @foreach ($patients_allergies as $patient_allergy)
                                <tr class="border-b hover:bg-orange-100">
                                    @foreach ($patient_allergy as $key => $value)
                                    ...
                                    <td class="text-red-500 p-3 px-5">
                                        <form id="delete-form" action="/controllers/patients_allergies/destroy.php" method="post" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                            <input type="hidden" name="patient" value="{{ $patient_allergy['patient'] }}">
                                            <input type="hidden" name="allergy" value="{{ $patient_allergy['allergy'] }}">
                                ...
                                <tr class="border-b hidden">
                                    <form id="edit-form" action="/controllers/patients_allergies/update.php" method="post">
                                        @foreach ($patient_allergy as $key => $value)
                                            @if (strpos($key, 'link-') === 0)
                                                <td class="p-3 px-5"></td>
                                            @elseif ($key != 'patient' && $key != 'allergy')
                                                <td class="p-3 px-5">
